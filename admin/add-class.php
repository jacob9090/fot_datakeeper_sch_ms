...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        $required = ['class_code', 'class_name'];
        ...
        $checkSql = "SELECT id FROM class_table WHERE LOWER(class_name) = LOWER(?)";
        $checkStmt = $conn->prepare($checkSql);
        $checkStmt->bind_param("s", $class_name);
        $checkStmt->execute();
        $checkStmt->store_result();
        if ($checkStmt->num_rows > 0) {
            $_SESSION['danger_alert'] = "Class already exists";
            $checkStmt->close();
        ...
        $sqlSchool = "INSERT INTO class_table (class_code, class_name) VALUES (?, ?)";
        $stmtSchool = $conn->prepare($sqlSchool);
        $stmtSchool->bind_param("ss", $class_code, $class_name);
        $stmtSchool->execute();

        $logSql = "INSERT INTO user_activity_log (user_id, first_name, last_name, action) VALUES (?, ?, ?, ?)";
        $action = "Added class: $class_name";
        ...
        $smsQuery = "SELECT message_to_send FROM sms_setup_table WHERE sms_type = 'Add Class' AND sms_group = 'Head'";
        ...
        if ($message && $school_mobile_number) {
            ...
            if (!$smsResponse['status']) {
                $_SESSION['danger_alert'] = "Class added, but SMS failed: " . $smsResponse['message'];
                header("Location: add-class.php");
                exit();
            }
        ...
        $_SESSION['success_alert'] = "Class added successfully. SMS sent.";
        header("Location: add-class.php");
        exit();
          ...
          <ul class="nav-main">
            ...
            <li class="nav-main-item open">
              ...
              <ul class="nav-main-submenu">
                ...
                <li class="nav-main-item">
                  <a class="nav-main-link active" href="add-class.php">
        ...
        <div class="row">
              ...
              <form action="add-class.php" method="POST" enctype="multipart/form-data">
                <div class="block block-rounded">
                  <div class="block-header block-header-default">
                    <h3 class="block-title">Add <small>Class</small></h3>
                    <div class="block-options">
                      <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#upload-class-modal"><i class="fa fa-upload"></i> Upload
                      </button>
                    </div>
                  </div>
                  <div class="block-content">
                    ...
                    <div class="form-floating mb-4">
                      <input type="text" class="form-control" id="example-text-input-floating" name="class_name" placeholder="Class1" required>
                      <label class="form-label" for="example-text-input-floating">Class Name</label>
                    </div>
                    
                    <div class="form-floating mb-4">
                      <input type="text" class="form-control" id="example-text-input-floating" name="class_code" placeholder="Class1" required>
                      <label class="form-label" for="example-text-input-floating">Class Code</label>
                    </div>

                    <div class="col-md-12">
                      <button type="submit" class="btn btn-hero btn-primary me-1 mb-3 btn-block w-100">
                        <i class="fa fa-fw fa-plus me-1"></i> Add Class
                      </button>
                    </div>
            ...
            <div class="col-md-8">
              <form action="send-text.php" method="POST" enctype="multipart/form-data">
                <div class="block block-rounded">
                  <div class="block-header block-header-default">
                    <h3 class="block-title">Class <small>List</small></h3>
                  </div>
                  <div class="block-content">
                    <div class="col-lg-12 col-xl-12">
                      mmm
          ...
          <div class="modal fade" id="upload-class-modal" tabindex="-1" role="dialog" aria-labelledby="upload-class-modal" aria-hidden="true">
            <div class="modal-dialog modal-dialog-popin" role="document">
              <div class="modal-content">
                <div class="block block-rounded block-themed block-transparent mb-0">
                  <div class="block-header bg-primary-dark">
                    <h3 class="block-title">Upload Class</h3>
                    <div class="block-options">
                  ...
                  <div class="block-content">
                    <div class="mb-4">
                      <input class="form-control" type="file" name="upload_class" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" id="example-file-input">
                    </div>
                    <button type="submit" class="btn btn-hero btn-primary me-1 mb-3 btn-block w-100">
                      <i class="fa fa-fw fa-upload me-1"></i> Upload Class
                    </button>
                  </div>
