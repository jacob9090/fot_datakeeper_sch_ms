...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $notice_group = $_POST["notice_group"];
    $msg_subject = $_POST["msg_subject"];
    $msg_content = $_POST["msg_content"];

    $sql = "INSERT INTO notice_table (notice_group, msg_subject, msg_content) 
            VALUES ('$notice_group', '$msg_subject', '$msg_content')";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        $_SESSION['notification'] = [
            'type' => 'danger',
            'message' => 'Error adding notice: ' . $conn->error
        ];
        header("Location: add-notice.php");
        exit;
    }
        ...
        <div class="content-side">
          <ul class="nav-main">
            ...
            <li class="nav-main-item open">
              ...
              <ul class="nav-main-submenu">
                <li class="nav-main-item">
                  <a class="nav-main-link active" href="add-notice.php">
      ...
      <div class="content">
        ...
        <div class="row">
              ...
              <form action="add-notice.php" method="POST" enctype="multipart/form-data">
                <div class="block block-rounded">
                  <div class="block-header block-header-default">
                    <h3 class="block-title">Add <small>Notice</small></h3>
                  </div>
                  <div class="block-content">
                    ...
                    <div class="form-floating mb-4">
                      <select class="form-select" id="example-select-floating" name="notice_group" aria-label="Floating label select example" required>
                        <option selected>Select Group</option>
                        <option value="Student">Student</option>
                        <option value="Teachers">Teachers</option>
                        <option value="students_teachers">Students/Teachers</option>
                      </select>
                      <label class="form-label" for="example-select-floating">Group</label>
                    </div>

                    <div class="form-floating mb-4">
                      <input type="text" class="form-control" id="example-text-input-floating" name="msg_subject" placeholder="Subject" required>
                      <label class="form-label" for="example-text-input-floating">Subject</label>
                    </div>

                    <div class="form-floating mb-4">
                      <textarea class="form-control" id="example-textarea-floating" name="msg_content" style="height: 200px" placeholder="Message here" required></textarea>
                      <label class="form-label" for="example-textarea-floating">Message</label>
                    </div>

                    <div class="col-md-12">
                      <button type="submit" class="btn btn-hero btn-primary me-1 mb-3 btn-block w-100">
                        <i class="fa fa-fw fa-plus me-1"></i> Add Notice
                      </button>
                    </div>
            ...
            <div class="col-md-8">
              <div class="block block-rounded">
                <div class="block-header block-header-default">
                  <h3 class="block-title">Notice <small>List</small></h3>
                </div>
                <div class="block-content block-content-full">
                  <div class="col-md-12 col-lg-12 col-xl-12">
                  <table class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination" id="notice-table">
                    <thead>
                      <tr>
          ...
          <!-- Edit Modal -->
          <div class="modal fade" id="edit-notice-modal" tabindex="-1" role="dialog" aria-labelledby="upload-student-modal" aria-hidden="true">
    ...
    $(document).ready(function () {
        ...
        const table = $('#notice-table').DataTable({
            ...
            initComplete: function() {
                $("#notice-table_filter input").attr("placeholder", "search");
            }
        });
                ...
                success: function (data) {
                    ...
                    data.forEach((notice, index) => {
                        table.row.add([
                            ...
                            notice.msg_subject,
                            notice.msg_content,
                            new Date(notice.on_create).toLocaleDateString(),
                            `<button class="btn btn-sm btn-primary edit-btn" data-id="${notice.id}">Edit</button>
                             <button class="btn btn-sm btn-danger delete-btn" data-id="${notice.id}">Delete</button>`
                        ]).draw(false);
                    });
                },
                error: function () {
                    $.notify("Failed to load notices", "danger");
                }
            });
        }
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            $.ajax({
                ...
                dataType: 'json',
                success: function (data) {
                    $('#edit-notice-id').val(data.id);
                    $('#edit-subject').val(data.msg_subject);
                    $('#edit-message').val(data.msg_content);
                    $('#edit-notice-modal').modal('show');
                },
                error: function () {
                    $.notify("Failed to fetch notice details", "danger");
                }
            });
        });

        // Handle form submission for editing
        $('#edit-notice-form').on('submit', function (e) {
            e.preventDefault();
            const id = $('#edit-notice-id').val();
            ...
            $.ajax({
                ...
                error: function () {
                    $.notify("Failed to update notice", "danger");
                }
            });
        });
        ...
        $('#confirm-delete-btn').on('click', function () {
            if (deleteNoticeId) {
                $.ajax({
                    ...
                    error: function () {
                        $.notify("Failed to delete notice", "danger");
                    }
                });
            }
        });
